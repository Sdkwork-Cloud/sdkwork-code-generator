...
describe('single-quoted query parameter name escaping', () => {
  ...
  const escapedName = "client\\'trace";
  const rawName = "client'trace";
  ...
  test.each(cases)(
    '$name escapes apostrophes in query parameter names embedded in single-quoted literals',
    ({ generator }) => {
      ...
      expect(code).toContain(escapedName);
      expect(code).not.toContain(rawName);
    }
  );
});
