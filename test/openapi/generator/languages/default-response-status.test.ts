...
describe('default response status support', () => {
  ...
  const cases: DefaultResponseCase[] = [
    {
      name: 'javascript/got',
    ...
    },
    {
      ...
      includes: ['if httpResponse.statusCode < 200 || httpResponse.statusCode >= 300 {'],
      excludes: ['if httpResponse.statusCode != 200 {'],
    },
    {
      name: 'dart/http',
      generator: new HttpDartRequestCodeGenerator(),
      includes: ['if (response.statusCode >= 200 && response.statusCode < 300) {'],
      excludes: ['if (response.statusCode == 200) {'],
    },
    {
      name: 'dart/dio',
    ...
    },
    {
      ...
      includes: ['if [ "$http_status" -lt 200 ] || [ "$http_status" -ge 300 ]; then'],
  ...
  for (const testCase of cases) {
    test(`uses generic 2xx success checks for ${testCase.name}`, () => {
